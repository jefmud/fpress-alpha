...
def cli_arg_exists(arg):
  """see if command line arg exists, no value check"""
  if arg in sys.argv:
    return True
  return False

def cli_arg_value(arg, default=None, required=False):
  """get a command line argument"""
  if arg in sys.argv:
    try:
      return args[args.index(arg) + 1]
    except:
      raise ValueError('missing argument for {}'.format(arg))
  if not required:
    return default
  else:
